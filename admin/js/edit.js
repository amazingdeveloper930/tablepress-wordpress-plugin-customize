    ...
    tp.table = {
        ...
        prepare_ajax_request: function(wp_action, wp_nonce) {
            ...
            $table_body.children().each(function(idx, row) {
                table_data[idx] = $(row).find('textarea.table-data')
                    .map(function() {
                        return $(this).val();
                    })
                    .get();
                // if ($id('table_isproduct').prop('checked')) {
                //     table_data[idx]['is_actived'] = $(row).find('input.checkbox_is_actived').prop('checked');
                // }
            ...
            if ($id('table_isproduct').prop('checked')) {
                table_detailed_data = $table_body.find('tr')
                    .map(function() {
                        return { is_actived: $(this).find('.checkbox_is_actived').prop('checked') };
                    }).get();
            }
            table_detailed_data = JSON.stringify(table_detailed_data);
            ...
            table_options = {
                ...
                is_product_table: $id('table_isproduct').prop('checked'),
                detailed_data: table_detailed_data,
            };
    ...
    tp.reindex = function() {
        ...
        $rows
            ...
            .each(function(row_idx, row) { // need a second loop here to not break logic in previous loop, that queries textareas by their old ID
                $(row).find('textarea.table-data').attr('id', function(column_idx /*, old_id */ ) {
